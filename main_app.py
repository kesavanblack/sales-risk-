...
if uploaded_file is not None:
    ...
    if menu == "Overview":
        ...
        fig = px.bar(country_counts, x=country_counts.index, y=country_counts.values, title="Customer Distribution by Country", labels={"x":"Country", "y":"Customer Count"})
        st.plotly_chart(fig)
        
        st.subheader("📅 Monthly Sales Trend")
        df_monthly_customers = df.resample("M", on="InvoiceDate")["CustomerID"].nunique()
        fig = px.line(df_monthly_customers, x=df_monthly_customers.index, y=df_monthly_customers.values, title="Monthly Unique Customers Trend", labels={"x":"Month", "y":"Unique Customers"})
        st.plotly_chart(fig)
